...
export function buildSpecFromAnswers(answers: Answer[]): string {
  ...
  // ─── Footer ──────────────────────────────────────────────────
  // File placement varies by tool — don't hardcode Copilot path here.
  // The how-it-works page covers where each tool expects its context file.
  sections.push(
    `---\n\n*Generated by [SpecifyThat](https://specifythat.com) on ${new Date().toISOString().slice(0, 10)}. ` +
    `Drop this file where your AI tool expects its context — see [specifythat.com/how-it-works](https://specifythat.com/how-it-works) for tool-specific placement.*`
  );
